...
import {
  FiUsers, FiSearch, FiRefreshCw, FiCheckCircle, FiXCircle,
  FiInbox, FiClock, FiX, FiEye, FiCreditCard, FiArrowLeft, FiCalendar,
  FiActivity, FiEdit2, FiToggleLeft, FiToggleRight
} from "react-icons/fi";
    ...
    <div className="min-h-screen font-sans pb-20 -mt-6 -mx-4 md:-mx-6" style={{ background: C.offWhite }}>
        ...
        <div className="w-full flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="flex items-center gap-6">
             ...
             <div className="flex items-center gap-5">
               ...
               <div>
                 <h1 className="text-3xl font-black tracking-tight leading-none">Corporate Access Control</h1>
      ...
      <div className="w-full px-4 md:px-10 -mt-10 space-y-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard label="Total Corporates" value={stats.total} Icon={FiUsers} borderCls="border-[#000D26]" iconBgCls="bg-slate-100" iconColorCls="text-[#000D26]" />
          <StatCard label="Active" value={stats.active} Icon={FiActivity} borderCls="border-emerald-500" iconBgCls="bg-emerald-50" iconColorCls="text-emerald-600" />
          <StatCard label="Inactive" value={stats.inactive} Icon={FiXCircle} borderCls="border-rose-500" iconBgCls="bg-rose-50" iconColorCls="text-rose-600" />
          <StatCard label="Total Credit Limit" value={`₹${stats.creditSum.toLocaleString('en-IN')}`} Icon={MdBusiness} borderCls="border-[#003399]" iconBgCls="bg-indigo-50" iconColorCls="text-indigo-600" />
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 border shadow-sm" style={{ borderColor: C.border }}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-5">
            ...
            <div className="flex items-end lg:col-span-1">
               <button onClick={() => { setSearch(""); setFilter("all"); setStartDate(""); setEndDate(""); setSelectedCorporateId("all"); }} title="Reset Filters" className="w-full py-2.5 rounded-xl font-black text-[13px] flex items-center justify-center border shadow-sm transition-all hover:bg-slate-100 hover:text-slate-700 bg-white text-slate-500 border-slate-200">
                  <FiX />
               </button>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border overflow-hidden" style={{ borderColor: C.border }}>
          <div className="p-5 border-b flex flex-wrap items-center justify-between gap-3" style={{ borderColor: C.border, background: C.white }}>
            <div>
              <h2 className="font-black text-slate-800 tracking-tight text-lg">Access Ledger</h2>
              <p className="text-[11px] font-bold uppercase tracking-widest mt-1" style={{ color: C.muted }}>{filtered.length} accessible records</p>
            </div>
            <TableActionBar
                      ...
                      <td className="px-6 py-5">
                        <div className="flex flex-col">
                          <span className="text-[11px] font-black text-slate-700">
                            {(() => {
                              const dateToCheck = c.onboardDate || c.updatedAt || c.createdAt;
