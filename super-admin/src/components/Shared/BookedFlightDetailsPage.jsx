...
import {
  ...
  fetchSuperAdminFlightBookingById as fetchMyBookingById,
...
/* ────────────────────────────────────────────────────────────── */
/*  New Layover banner component                                  */
/* ────────────────────────────────────────────────────────────── */
function LayoverBanner({ seg, nextSeg, layoverMinutes, formatLayoverDuration, formatDate, formatTime }) {
  const [open, setOpen] = useState(false);
  const minutes = layoverMinutes(seg, nextSeg);

  return (
    <div className="flex flex-col bg-white/60 border border-[#E0D8C8] rounded-xl overflow-hidden">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center gap-3 px-4 py-2.5 cursor-pointer outline-none w-full text-left"
      >
        ...
        <div className="flex items-center gap-3 text-[11px] text-[#8B7355] shrink-0">
          {seg.destination?.terminal && nextSeg?.origin?.terminal && (
            <span className="font-medium">
              T{seg.destination.terminal} → T{nextSeg.origin.terminal}
            </span>
          )}
          <span className="flex items-center gap-1 font-semibold">
            <FiClock size={11} />
            {formatLayoverDuration(minutes)}
          </span>
          <span className="text-[#A07840] ml-1">
      ...
      {open && (
        <div className="px-4 pb-3 pt-2 border-t border-[#E0D8C8]/50 text-[11px] text-gray-700 bg-white/40 space-y-3">
           <div className="flex flex-col gap-1 text-xs">
             <div className="flex justify-between items-center text-[#8B7355] font-medium">
               <span>Arriving flight</span>
               <span>{formatDate(seg.arrivalDateTime)}</span>
             </div>
             <div className="flex justify-between items-center text-gray-900 font-semibold text-[13px]">
               <span>{seg.origin?.city} ({seg.origin?.airportCode})</span>
               <span>{formatTime(seg.arrivalDateTime)}</span>
             </div>
             <span className="text-gray-500">{seg.airlineName} · {seg.airlineCode} {seg.flightNumber}</span>
           </div>

           <div className="w-full border-t border-dashed border-[#E0D8C8]" />
           
           <div className="flex flex-col gap-1 text-xs">
             <div className="flex justify-between items-center text-[#8B7355] font-medium">
               <span>Departing flight</span>
               <span>{formatDate(nextSeg.departureDateTime)}</span>
             </div>
             <div className="flex justify-between items-center text-gray-900 font-semibold text-[13px]">
               <span>{nextSeg.destination?.city} ({nextSeg.destination?.airportCode})</span>
               <span>{formatTime(nextSeg.departureDateTime)}</span>
             </div>
             <span className="text-gray-500">{nextSeg.airlineName} · {nextSeg.airlineCode} {nextSeg.flightNumber}</span>
           </div>
        </div>
      )}
    </div>
          ...
          <div className="mt-4 space-y-2">
            {segments.slice(0, -1).map((seg, i) => (
              <LayoverBanner
                ...
                formatLayoverDuration={formatLayoverDuration}
...
function hasFareValue(obj, key) {
  return Object.prototype.hasOwnProperty.call(obj || {}, key);
}

function firstFareValue(obj, ...keys) {
...
function getItineraryFare(bookingResult, journeyKey) {
  const responseKey =
    journeyKey === "return" ? "returnResponse" : "onwardResponse";
  return safeGet(
    bookingResult,
    responseKey,
    "Response",
...
function collectFareSections({ fareSnapshot, fareQuoteResults, bookingResult }) {
  const quoteResults = Array.isArray(fareQuoteResults) ? fareQuoteResults : [];
  const sections = [];

  const pushSection = (label, fare, fallbackFare = {}) => {
    const merged = Object.fromEntries(
      Object.entries({ ...(fallbackFare || {}), ...(fare || {}) }).filter(
        ([, value]) => value !== undefined && value !== null && value !== "",
      ),
    );
    if (Object.keys(merged).length) sections.push({ label, fare: merged });
  };
  ...
  if (fareSnapshot?.onwardFare || fareSnapshot?.returnFare) {
    pushSection(
      ...
      getItineraryFare(bookingResult, "onward"),
    );
    pushSection(
      "Return Fare",
      fareSnapshot.returnFare,
      getItineraryFare(bookingResult, "return"),
    );
  } else {
    const providerFare =
      safeGet(
    ...
    pushSection("Ticket Fare", {
      BaseFare: fareSnapshot?.baseFare,
      Tax: fareSnapshot?.tax,
      PublishedFare: fareSnapshot?.publishedFare,
      OfferedFare: fareSnapshot?.offeredFare,
      ServiceFee: fareSnapshot?.serviceFee,
      OtherCharges: fareSnapshot?.otherCharges,
      Currency: fareSnapshot?.currency,
      IsRefundable: fareSnapshot?.refundable,
      ...(providerFare || {}),
    });
  }
  ...
  if (!sections.length) {
    quoteResults.forEach((result, index) => {
      pushSection(`Fare Quote ${index + 1}`, result?.Fare);
    });
  }
...
function paxTypeLabel(type) {
  const numericMap = { 1: "Adult", 2: "Child", 3: "Infant" };
  return numericMap[type] || formatPaxType(type);
}

function FareSummarySection({
  booking,
  pricingSnap,
  fareSnapshot,
  fareQuoteResults,
  ...
  const currency =
    pricingSnap?.currency ||
    fareSnapshot?.currency ||
    fareSnapshot?.onwardFare?.Currency ||
    fareQuoteResults?.[0]?.Fare?.Currency ||
    "INR";

  const fareSections = collectFareSections({
    fareSnapshot,
    fareQuoteResults,
    bookingResult,
  });
  const fareBreakdown = (fareQuoteResults || []).flatMap(
    (result) => result?.FareBreakdown || [],
  );
  ...
  const baseTotal = fareSections.reduce(
    (sum, section) =>
      sum + Number(firstFareValue(section.fare, "BaseFare", "baseFare") || 0),
    0,
  );
  const taxTotal = fareSections.reduce(
    (sum, section) =>
      sum + Number(firstFareValue(section.fare, "Tax", "tax") || 0),
    0,
  );
  const refundable =
    ...
    fareSections.some((section) => section.fare?.IsRefundable);

  const fareFields = [
    ["Base Fare", "BaseFare", "baseFare"],
    ["Tax", "Tax", "tax"],
    ["YQ Tax", "YQTax"],
    ["Published Fare", "PublishedFare", "publishedFare"],
    ["Offered Fare", "OfferedFare", "offeredFare"],
    ["Other Charges", "OtherCharges", "otherCharges"],
    ["Service Fee", "ServiceFee", "serviceFee"],
    ["Transaction Fee", "TransactionFee"],
    ["PG Charge", "PGCharge"],
    ["Commission Earned", "CommissionEarned"],
    ["PLB Earned", "PLBEarned"],
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <div className="bg-slate-900 rounded-xl p-4 text-white">
          <p className="text-[9px] font-bold uppercase tracking-widest text-slate-300 mb-1">
            Total Price
          </p>
          <p className="text-2xl font-black">
            {formatMoney(publishedTotal, currency)}
          </p>
        </div>
        ...
        <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
          ...
          <p
            className={`text-lg font-black ${
              refundable ? "text-emerald-700" : "text-red-600"
            }`}
          >
            {refundable ? "Yes" : "No"}
          </p>
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5">
                {fareFields
                  .map(([label, ...keys]) => [
                    label,
                    firstFareValue(section.fare, ...keys),
                    keys,
                  ])
                  .filter(
                    ([, value, keys]) =>
                      value !== undefined ||
                      keys.some((key) => hasFareValue(section.fare, key)),
                  )
                  .map(([label, value]) => (
                    <InfoRow
                      key={label}
      ...
      {fareBreakdown.length > 0 && (
        <div className="mt-5">
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#A07840] mb-3">
            Passenger Price Breakdown
          </p>
          <div className="space-y-3">
            {fareBreakdown.map((fb, i) => {
              const fbCurrency = fb.Currency || currency;
              const taxBreakup = fb.TaxBreakUp || fb.TaxBreakup || [];
              return (
                <div
                  key={i}
                ...
                >
                  <div className="px-4 py-2.5 bg-gray-50 border-b border-gray-100 flex items-center gap-3">
                    <span className="text-[11px] font-bold uppercase tracking-widest text-gray-700">
                      {paxTypeLabel(fb.PassengerType)} x{" "}
                      {fb.PassengerCount || 1}
                    </span>
                  ...
                  <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-4">
                    {[
                      ["Base Fare", fb.BaseFare],
                      ["Tax", fb.Tax],
                      ["YQ Tax", fb.YQTax],
                      ["Transaction Fee", fb.TransactionFee],
                      ["PG Charge", fb.PGCharge],
                      ["Additional Txn Fee Pub", fb.AdditionalTxnFeePub],
                      ["Additional Txn Fee Offered", fb.AdditionalTxnFeeOfrd],
                      ["Supplier Reissue Charges", fb.SupplierReissueCharges],
                    ].map(([label, value]) => (
                      <InfoRow
                        key={label}
                        label={label}
                        value={formatMoney(value || 0, fbCurrency)}
                      />
                    ))}
                  </div>
                  {taxBreakup.length > 0 && (
      ...
      {hasSSR && (
        <div className="mt-5 bg-[#FAF7F2] border border-[#E8E0D0] rounded-xl p-4">
          ...
          {[
            ...seats.map((s) => ({
              key: `seat-${s.travelerIndex}-${s.segmentIndex}-${s.seatNo}`,
              label: `Seat - ${s.seatNo}`,
              meta: `${travelerName(s.travelerIndex)}${
                segLabel(s.segmentIndex) ? ` - ${segLabel(s.segmentIndex)}` : ""
              }`,
              price: s.price || 0,
            })),
            ...meals.map((m) => ({
              key: `meal-${m.travelerIndex}-${m.segmentIndex}-${m.code}`,
              label: `Meal - ${m.code}`,
              meta: `${travelerName(m.travelerIndex)}${
                segLabel(m.segmentIndex) ? ` - ${segLabel(m.segmentIndex)}` : ""
              }`,
              price: m.price || 0,
            })),
            ...baggage.map((b) => ({
              key: `bag-${b.travelerIndex}-${b.segmentIndex}-${
                b.weight || b.description
              }`,
              label: `Baggage - ${b.weight || b.description || "Extra"}`,
              meta: `${travelerName(b.travelerIndex)}${
                segLabel(b.segmentIndex) ? ` - ${segLabel(b.segmentIndex)}` : ""
              }`,
              price: b.price || 0,
            })),
          ].map((item) => (
          ...
          <div className="flex justify-between items-center mt-2.5 pt-2 border-t border-dashed border-[#D8CEB8] text-[12px]">
            <span className="text-gray-500 font-semibold">SSR Total</span>
            <span className="font-semibold text-gray-900">
              {formatMoney(ssrTotal, currency)}
            </span>
          </div>
        </div>
      )}
...
function normalizeCancellationResponses(booking) {
  const candidates = [
    booking?.amendment?.response,
    booking?.amendment?.raw,
    ...(booking?.amendmentHistory || []).flatMap((item) => [
      item?.response,
      item?.raw,
    ]),
  ];
  ...
  const addRow = (item) => {
    ...
    const responseRoot = item?.response || item;
    const root = responseRoot?.Response || responseRoot || {};
    const ticketInfo = Array.isArray(root?.TicketCRInfo)
      ? root.TicketCRInfo[0]
      : root?.TicketCRInfo;
    ...
    rows.push({
      bookingId: item?.bookingId || responseRoot?.bookingId || root?.BookingId,
      info: ticketInfo,
    });
  };

  candidates.forEach(addRow);

  const seen = new Set();
  return rows.filter(({ bookingId, info }) => {
    const key = [
      bookingId,
...
function getCancellationChangeRequestIds(booking) {
  const ids = new Set();
  [
    booking?.changeRequestId,
    booking?.cancellationRequestId,
    booking?.amendment?.changeRequestId,
    booking?.amendment?.cancellationRequestId,
  ].forEach((changeRequestId) => {
    if (changeRequestId) ids.add(changeRequestId);
  });

  normalizeCancellationResponses(booking).forEach(({ info }) => {
    if (info?.ChangeRequestId) ids.add(info.ChangeRequestId);
  });
...
/*  Main page component                                            */
/* ─────────────────────────────────────────────────────────────── */
export default function BookedFlightDetails() {
  const { id } = useParams();
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const passengerTableRef = useRef(null);
  const cancellationStatusCheckedRef = useRef(new Set());
  ...
  const {
    selectedFlightBooking: booking,
    loadingSelectedFlightBooking: loading,
  ...
  useEffect(() => {
    if (!booking?._id || !isCancelled) return;

    const changeRequestIds = getCancellationChangeRequestIds(booking);
    if (!changeRequestIds.length) return;

    const checkKey = `${booking._id}:${changeRequestIds.join(",")}`;
    if (cancellationStatusCheckedRef.current.has(checkKey)) return;
    cancellationStatusCheckedRef.current.add(checkKey);

    let cancelled = false;

    const refreshCancellationStatus = async () => {
      try {
        await Promise.all(
        ...
        console.error("Failed to refresh cancellation status", err);
      }
    };
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              {booking.payment?.paymentId && (
                <span className="text-xs font-mono bg-amber-50 border border-amber-200 text-amber-700 px-2 py-0.5 rounded-md">
                  <span className="font-semibold">PID:</span> {booking.payment.paymentId}
                </span>
              )}
      ...
      <main className="w-full px-4 lg:px-10 py-8 pb-24 space-y-6">
        ...
        {(isCancelled || booking.cancellation) && (() => {
          ...
          if (cancellationRows.length > 0) {
            cancellationRows.forEach(({ bookingId, info }) => {
              ...
              sectorBreakdown.push({
                label: getSectorLabel(bookingId),
                refund: info.RefundedAmount,
                charge: info.CancellationCharge,
                creditNote: info.CreditNoteNo,
                remarks: info.Remarks,
                creditNoteCreatedOn: info.CreditNoteCreatedOn,
              });
            });
          } else {
            totalRefund = Number(booking.amendment?.refundedAmount || 0);
            totalCharge = Number(booking.amendment?.cancellationCharge || 0);
          }
          ...
          const displayRemarks = providerRemarks.length > 0 ? providerRemarks.join(" | ") : (cancellationRows.length > 0 ? "Successful" : "");
          return (
            <div className="space-y-6 mt-8">
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {(displayRemarks || cancellationRows.length > 0) && (
                    <div className="flex items-start gap-3">
        ...
        {paymentSuccessful && executionStatus === "ticketed" && !isCancelled && !isTravelPassed && !booking.cancellation ? (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  {/* ── Dynamic reissue button based on eligibility ── */}
                  {eligibilityLoading ? (
