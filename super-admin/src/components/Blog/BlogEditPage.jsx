  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-[#F8FAFC]">
        <FaSyncAlt className="animate-spin w-10 h-10 text-[#003399] mb-4" />
        ...
        <div className="w-full flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h1 className="text-3xl font-black tracking-tight leading-none">Edit Article</h1>
                  <p className="text-[10px] mt-2 font-bold uppercase tracking-[2px] opacity-60">
                    Refine your published content
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              <button type="button" onClick={() => setShowPreview(true)} className="flex items-center justify-center space-x-2 px-5 py-2.5 bg-white/10 hover:bg-white/20 border border-white/20 text-white rounded-xl text-xs font-black uppercase tracking-wider transition-all shadow-sm">
                <FaEye className="w-4 h-4" /><span>Preview</span>
              </button>
              <button type="button" onClick={(e) => handleSubmit(e, "draft")} disabled={saving} className="flex items-center justify-center space-x-2 px-5 py-2.5 bg-slate-800 text-white rounded-xl hover:bg-slate-900 transition-all shadow-md text-xs font-black uppercase tracking-wider disabled:opacity-50">
                <FaSave className="w-4 h-4" /><span>{saving ? "Saving..." : "Save Draft"}</span>
              </button>
              <button type="button" onClick={(e) => handleSubmit(e, "published")} disabled={saving} className="flex items-center justify-center space-x-2 px-5 py-2.5 bg-white text-[#003399] rounded-xl hover:bg-slate-100 transition-all shadow-sm text-xs font-black uppercase tracking-wider disabled:opacity-50">
                <FiPlus className="w-4 h-4" /><span>{saving ? "Updating..." : "Update & Publish"}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="flex border-b border-slate-100">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-1 flex items-center justify-center space-x-2 py-4 text-[11px] font-black uppercase tracking-widest transition-all ${activeTab === tab.id ? "bg-slate-50 border-b-2 border-[#003399] text-[#003399]" : "text-slate-400 hover:text-slate-600 hover:bg-slate-50/50"}`}>
              ...
              {activeTab === "content" && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className="flex flex-col gap-2">
                    ...
                    <input type="text" name="title" value={formData.title} onChange={handleChange} className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#003399]/20 focus:border-[#003399] outline-none text-xl font-bold text-slate-800" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Featured Image</label>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <div className="relative h-48 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50 flex flex-col items-center justify-center group hover:border-[#003399] cursor-pointer overflow-hidden">
                          {imagePreview ? (
                            <>
                      ...
                      <div className="bg-slate-50 rounded-xl border border-slate-100 p-4 flex flex-col justify-center text-xs text-slate-500 space-y-2">
                        ...
                        <p>Current Status: <span className="font-bold uppercase text-[#003399]">{formData.status}</span></p>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="space-y-6">
                      <div className="flex flex-col gap-2">
                        ...
                        <input type="text" name="meta_title" value={formData.meta_title} onChange={handleMetaTitleChange} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-[#003399]/20 focus:border-[#003399] text-sm font-medium" />
                      </div>
                      <div className="flex flex-col gap-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Meta Description</label>
                        <textarea name="meta_description" value={formData.meta_description} onChange={handleChange} rows={4} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-[#003399]/20 focus:border-[#003399] text-sm font-medium resize-none" />
                      </div>
                    </div>
                    <div className="space-y-6">
                      <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="relative">
                          ...
                          <input type="text" value={formData.categories[0] || ""} onChange={handleCategoryChange} className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-[#003399]/20 focus:border-[#003399] text-sm font-bold" />
                        </div>
                      </div>
                      <div className="flex flex-col gap-2">
              ...
              {activeTab === "settings" && (
                <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className="max-w-xl bg-slate-50 rounded-2xl border border-slate-100 p-8 space-y-6">
                    <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight flex items-center gap-2"><FaUser className="text-[#003399]" /> Author Configuration</h3>
                    <div className="space-y-4">
                      <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <input type="text" name="author_name" value={formData.author_name} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-[#003399]/20 focus:border-[#003399]" />
                      </div>
                    </div>
                  </div>
                </div>
