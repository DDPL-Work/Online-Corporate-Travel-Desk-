...
import { FaPlane, FaHotel, FaStar } from "react-icons/fa";
import { useNavigate } from "react-router-dom";

export default function ServiceFeesModal({ isOpen, onClose, rules = [], corporate }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-7xl max-h-[90vh] flex flex-col overflow-hidden relative">
        <button 
          onClick={onClose}
          ...
          <div className="flex items-center justify-between pr-16">
            ...
            {corporate && (
              <button
                onClick={() => {
                  onClose();
                  navigate(`/edit-corporate/${corporate._id}?tab=service_fee`);
                }}
                className="flex items-center gap-2 px-5 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-xl text-sm font-black uppercase tracking-widest transition-all backdrop-blur-sm border border-white/20 shadow-sm"
              >
                <FiEdit2 size={16} />
                Edit Fees
              </button>
