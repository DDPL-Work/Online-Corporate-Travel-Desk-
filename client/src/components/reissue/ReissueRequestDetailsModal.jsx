...
import {
  FiCheckCircle, FiClock, FiDownload, FiFileText,
  FiRefreshCw, FiRepeat, FiX,
} from "react-icons/fi";
...
function LabelValue({ label, value, mono = false, accent = false }) {
  return (
    <div className="rounded-xl border border-slate-100 bg-slate-50 p-3">
      ...
      <p className={`mt-1 text-sm font-semibold ${mono ? "font-mono" : ""} ${accent ? "text-[#0A4D68]" : "text-slate-800"}`}>
        {value || "N/A"}
      </p>
    </div>
  );
...
export default function ReissueRequestDetailsModal({ requestId, onClose }) {
  ...
  const handleConfirm = async () => {
    try {
      ...
      const msg = typeof err === "string" ? err : err?.message || "Failed to confirm reissue";
      toast.error(`⚠️ ${msg}`);
    }
  };
  ...
  /* ── Ticket / invoice URLs ── */
  const ticketUrl = getTicketUrl(request) ||
      ...
      <div className="max-h-[90vh] w-full max-w-5xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black text-slate-900">Reissue Tracker</h2>
          ...
          {detailLoading || !request ? (
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className={`rounded-full border px-3 py-1 text-[11px] font-black uppercase tracking-wider ${getStatusTone(getStatus(request))}`}>
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <LabelValue label="Reissue ID"    value={safeVal(request.reissueId)}   mono />
                <LabelValue label="Original PNR"  value={pnr}                           mono />

                <LabelValue label="Airline"        value={getAirline(request)}                          />
                <LabelValue label="Last Updated"   value={safeDateTime(request.updatedAt)}         />
              </div>
                ...
                <div className="rounded-2xl border border-slate-200 p-5">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    ...
                    <LabelValue
                      ...
                      value={getJourneyLabel(request.newJourney) !== "N/A"
                    ...
                    <LabelValue
                      label="New Departure"
                      value={safeDate(request.newJourney?.departureDate || request.preferredDate)}
                    />
                    <LabelValue
                      label="New Return"
                ...
                <div className="rounded-2xl border border-slate-200 p-5">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <LabelValue
                      ...
                      value={request.fareDifference != null ? safeMoney(request.fareDifference) : "N/A"}
                    />
                    <LabelValue
                      label="Reissue Charges"
                      value={request.reissueCharges != null ? safeMoney(request.reissueCharges) : "N/A"}
                    />
                    <LabelValue
                      label="Total Adjustment"
                      value={request.totalAdjustment != null ? safeMoney(request.totalAdjustment) : "N/A"}
                      accent
                    />
                    <LabelValue
                    ...
                    {request.opsRemarks.map((item, idx) => (
                      ...
                      >
                        ...
                        <p className="mt-1 text-xs text-slate-400">
                          {prettifyLabel(item.byRole)} • {safeDateTime(item.at)}
                        </p>
                      </div>
                    ))}
                  </div>
                ...
                {timeline.length === 0 ? (
                  ...
                  <div className="mt-4 space-y-4">
                    {timeline.map((item, idx) => (
                      <div key={`${item.at || idx}-${idx}`} className="flex gap-3">
                        <div className="flex flex-col items-center">
                          <span className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-600">
                            {idx === 0 ? <FiCheckCircle size={16} /> : <FiClock size={16} />}
                          </span>
                        ...
                        <div className="pb-4">
                          <p className="text-sm font-bold text-slate-800">
                            {item.title || prettifyLabel(item.status) || "Event"}
                          </p>
                          <p className="mt-1 text-sm text-slate-500">
