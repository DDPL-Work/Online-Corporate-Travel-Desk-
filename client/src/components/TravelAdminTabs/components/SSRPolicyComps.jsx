...
export function Toggle({ label, description, icon: Icon, enabled, onChange }) {
  return (
    <div
    ...
    >
      <div className="flex items-center gap-4">
        <div
          ...
          style={{
            background: enabled ? C.gold : C.offWhite,
        ...
        <div>
          <p
            ...
            style={{ color: enabled ? C.navy : C.muted }}
          >
            {label}
          </p>
...
export function LimitDropdown({ isUnlimited, onChange }) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div
          className="absolute top-full left-0 mt-1 w-full bg-white border shadow-lg rounded-xl overflow-hidden z-20 animate-in fade-in zoom-in-95 duration-100"
          style={{ borderColor: C.border }}
        >
          <button
  ...
  return (
    <div className="space-y-3">
      ...
      {items.map((item, idx) => (
        <div
          key={idx}
          className="flex items-center justify-between p-2 rounded-xl border bg-slate-50 shadow-sm"
          style={{ borderColor: C.border }}
        >
          <span className="text-xs font-bold text-slate-600 pl-2">
            {formatCondition
              ? formatCondition(item[conditionsKey])
              : item[conditionsKey]}
          </span>
          <div className="flex items-center gap-2">
            <LimitDropdown
              ...
              onChange={(val) =>
                updateIsUnlimited(location, item[conditionsKey], val)
              }
            />
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      {isOpen && filtered?.length > 0 && !isExactSelected && (
        <div
          className="absolute z-[60] w-full mt-2 bg-white rounded-2xl shadow-2xl border overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200"
          style={{ borderColor: C.border }}
        >
          <div className="max-h-64 overflow-y-auto w-full p-2 custom-scrollbar">
            {filtered.map((emp) => (
              ...
              >
                <div
                  className="w-10 h-10 rounded-xl flex items-center justify-center text-[11px] font-black text-white uppercase shadow-sm"
                  style={{
...
export function PolicyDetailModal({ policy, onClose }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] bg-[#000D26]/80 backdrop-blur-md flex items-center justify-center p-6"
      onClick={onClose}
    >
      <div
        className="w-full max-w-xl bg-white rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-300"
      ...
      >
        ...
        >
          ...
          <div className="mt-3 flex flex-col items-center justify-center gap-2">
            <StatusBadge
              ...
              color={policy.approvalRequired ? "amber" : "green"}
            />
        ...
        <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          <Detail label="Active Permissions" icon={FiCheckCircle}>
            <div className="flex flex-wrap gap-2">
              <StatusBadge
                text="Seat"
                color={policy.allowSeat ? "green" : "red"}
              />
              <StatusBadge
                text="Meal"
                color={policy.allowMeal ? "green" : "red"}
              />
              <StatusBadge
                text="Baggage"
                color={policy.allowBaggage ? "green" : "red"}
              />
            </div>
          </Detail>
                  ...
                  return (
                    <div key={type} className="text-center">
                      ...
                      <p
                        ...
                        style={{ color: allowed ? C.navy : "#CBD5E1" }}
                      >
        ...
        <div className="px-8 pb-8">
          <button
            onClick={onClose}
            className="w-full py-4 bg-slate-100 hover:bg-slate-200 text-slate-700 font-black text-xs uppercase tracking-widest rounded-2xl transition-all"
          >
            Dismiss
          </button>
        </div>
  ...
  return (
    <ResponsiveDataTable
      ...
      onExport={() =>
        exportExcel({
          pageHeader: title,
          statCards: [{ label: "Active Policies", value: policies.length }],
          appliedFilters: [{ label: "Search", value: searchQuery || "None" }],
          data: policies,
          ...
          {paginatedPolicies.map((p, i) => (
            ...
            >
              ...
              <td className="!px-6 !py-5 text-center">
                ...
                />
                {p.allowSeat && (
                  <p className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-tighter font-mono">
              ...
              <td className="!px-6 !py-5 text-center">
                <StatusBadge
                  text={p.approvalRequired ? "Manual" : "Auto"}
