...
function FlightSection() {
  ...
  useEffect(() => { dispatch(getAllFlightBookingsAdmin()); }, [dispatch]);

  const formattedBookings = useMemo(() => (flightBookings || []).map(b => {
    const traveller = b.userId?.name ? `${b.userId.name.firstName} ${b.userId.name.lastName}`.trim() : "Staff Member";
  ...
  const filtered = useMemo(() => {
    ...
    return formattedBookings.filter(b => {
      if (b.executionStatus !== "ticketed" || b.amendment?.status === "requested") return false;
      const booked = b.bookedDate ? new Date(b.bookedDate).toISOString().slice(0, 10) : "";
      return (!q || b.travellerName?.toLowerCase().includes(q) || b.employeeId?.toLowerCase().includes(q) || b.pnr?.toLowerCase().includes(q)) &&
...
function HotelSection() {
  ...
  useEffect(() => { dispatch(getAllHotelBookingsAdmin()); }, [dispatch]);

  const formattedBookings = useMemo(() => (hotelBookings || []).map(b => {
    const guest = b.userId?.name ? `${b.userId.name.firstName} ${b.userId.name.lastName}`.trim() : "Staff Member";
...
export default function TotalBookings() {
  ...
  const handleRefresh = async () => {
    setIsSyncing(true);
    try {
      await dispatch(activeTab === "flight" ? getAllFlightBookingsAdmin() : getAllHotelBookingsAdmin());
    } finally {
      setIsSyncing(false);
    }
  };
        ...
        <div className="w-full flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="flex items-center gap-6">
             ...
             <div className="flex items-center gap-5">
               ...
               <div>
                 <h1 className="text-3xl font-black tracking-tight leading-none">Total Booking Registry</h1>
                 <p className="text-[10px] mt-2 font-bold uppercase tracking-[2px] opacity-60">
                   Comprehensive Oversight of all Corporate Travel Deployments and Asset Management
                 </p>
               </div>
             </div>
          </div>
