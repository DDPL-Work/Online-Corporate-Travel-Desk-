...
import {
  FiCheck, FiCheckCircle, FiClock, FiDownload,
  FiEye, FiRefreshCw, FiRepeat, FiXCircle,
} from "react-icons/fi";
...
import LegacyReissueDetailModal from "../EmployeeDashboard/LegacyReissueDetailModal";
...
const TicketActions = ({ req }) => {
  ...
  if (!downloadUrl && !["COMPLETED", "TICKET_GENERATED"].includes(status)) {
    return <span className="text-[10px] font-bold text-slate-400 italic">Ticket Pending</span>;
  }
  
  if (!downloadUrl) {
    return <span className="text-[10px] font-bold text-slate-400 italic">Processing</span>;
  }
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <a href={downloadUrl} download={`reissue-${getRequestId(req)}.pdf`}
...
export default function ReissueRequests() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [companyRequests, legacyRequests] = await Promise.all([
        ...
        fetchSafe("/api/v1/flights/reissue/list", { companyId: corporateId, limit: 100 }) // Legacy fallback
      ]);
      ...
      setRequests(Array.from(uniqueMap.values()).sort((a, b) => new Date(getRequestedDate(b)) - new Date(getRequestedDate(a))));
    } catch (err) {
      console.error(err);
  ...
  const handleStatusUpdate = async (requestId, newStatus) => {
    ...
    try {
      await dispatch(updateReissueStatus({ requestId, status: newStatus, message })).unwrap();
  ...
  const stats = useMemo(() => ({
    ...
    approved:  requests.filter((r) => ["APPROVED", "COMPLETED", "TICKET_GENERATED", "IN_PROGRESS"].includes(getStatus(r))).length,
  ...
  const filtered = useMemo(() => {
    ...
    if (statusFilter !== "All") {
      ...
      if (statusFilter === "APPROVED") {
        result = result.filter(r => ["APPROVED", "COMPLETED", "TICKET_GENERATED", "IN_PROGRESS", "ASSIGNED"].includes(getStatus(r)));
      } else {
  ...
  const handleExport = () => {
    const rows = [
      ["Request ID", "PNR", "Booking Ref", "Employee", "Route", "Type", "Reason", "Date", "Status"],
      ...filtered.map((r) => [
        getRequestId(r),
        getPnr(r),
    ...
    a.href = url; a.download = "reissue_requests.csv"; a.click();
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black text-slate-900">Reissue Requests</h2>
              <p className="text-xs text-slate-400">Flight amendment and date-change requests</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={handleExport} className="px-3 py-2 text-[11px] font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50">
              Export CSV
            </button>
            <button onClick={() => load()} className="flex items-center gap-2 px-3 py-2 text-[11px] font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50">
        ...
        {loading ? (
            ...
            <div className="py-20 text-center bg-white rounded-2xl shadow-sm border border-slate-200">
              ...
              <p className="text-sm font-bold text-slate-400">No reissue requests found</p>
            </div>
          ) : (
                ...
                <table className="w-full border-collapse min-w-[1000px]">
                  <thead>
                    <tr className="bg-[#dac448] text-slate-900 border-b border-slate-200">
                      <th className="px-4 py-3.5 text-left text-[10px] font-black uppercase tracking-widest">Request ID</th>
                      ...
                      <th className="px-4 py-3.5 text-left text-[10px] font-black uppercase tracking-widest">Route</th>
                      <th className="px-4 py-3.5 text-left text-[10px] font-black uppercase tracking-widest">Type & Reason</th>
                      <th className="px-4 py-3.5 text-left text-[10px] font-black uppercase tracking-widest">Status</th>
                      <th className="px-4 py-3.5 text-left text-[10px] font-black uppercase tracking-widest">Ticket</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filtered.map((r, i) => {
                      ...
                      const bookingRef =
                        ...
                        "N/A";
                      return (
                        <tr key={requestId} className={`hover:bg-sky-50/40 transition-colors ${i % 2 === 0 ? "bg-white" : "bg-slate-50/30"}`}>
                          <td className="px-4 py-4">
                            <p className="font-mono text-[12px] font-bold text-slate-800">{requestId}</p>
                            <p className="text-[10px] text-slate-400 mt-1">{requestedAt}</p>
                          </td>
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-3">
                          ...
                          <td className="px-4 py-4">
                            ...
                            <p className="text-[11px] text-slate-500 mt-1.5 italic line-clamp-1 max-w-[200px]" title={r.reason || r.remarks || "N/A"}>
                          ...
                          <td className="px-4 py-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              {status === "PENDING" && (
                                <>
                                  <button
                                    onClick={() => handleStatusUpdate(r._id || r.id || r.requestId, "APPROVED")}
                                  ...
                                  <button
                                    onClick={() => handleStatusUpdate(r._id || r.id || r.requestId, "REJECTED")}
