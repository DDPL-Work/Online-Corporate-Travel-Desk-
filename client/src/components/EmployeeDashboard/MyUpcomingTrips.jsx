...
import { myUpcomingFlightsExportTemplate, myUpcomingHotelsExportTemplate } from "../../templates/exportTemplates/clientExportTemplates";
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <ResponsiveDataTable 
        ...
        onExport={() => exportExcel({
          pageHeader: "Upcoming Flight Ledger",
          statCards: [
            { label: "Upcoming Flights", value: filtered.length },
            { label: "Confirmed", value: filtered.length },
            { label: "Future Assets", value: filtered.length },
            { label: "Reserved Capital", value: `₹${totalValue.toLocaleString()}` }
          ],
          appliedFilters: [
          ...
          data: filtered,
          columns: myUpcomingFlightsExportTemplate,
          filenamePrefix: "my_upcoming_flights"
        })}
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <ResponsiveDataTable 
        ...
        exportDisabled={isExporting}
        onExport={() => exportExcel({
          pageHeader: "Upcoming Hotel Ledger",
          statCards: [
            { label: "Upcoming Hotels", value: filtered.length },
            { label: "Confirmed", value: filtered.length },
            { label: "Unique Destinations", value: new Set(filtered.map(b => b.city)).size },
            { label: "Reserved Capital", value: `₹${totalValue.toLocaleString()}` }
          ],
          appliedFilters: [
            { label: "Search", value: search || "None" },
          ...
          data: filtered,
          columns: myUpcomingHotelsExportTemplate,
          filenamePrefix: "my_upcoming_hotels"
        })}
        wrapperClass="!border-none !shadow-none" 
        pagination={<Pagination currentPage={currentPage} totalItems={filtered.length} pageSize={PAGE_SIZE} onPageChange={setCurrentPage} />}
      >
        ...
        <div className="w-full flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="flex flex-col md:flex-row md:items-center gap-6">
             ...
             <div className="flex items-center md:items-center gap-4 md:gap-5">
               <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl flex shrink-0 items-center justify-center shadow-xl text-white border border-white/10 bg-white/10"><FiCalendar size={24} className="md:w-7 md:h-7" /></div>
               <div>
                 <h1 className="text-3xl md:text-4xl font-black tracking-tight leading-none">My Upcoming Trips</h1>
                 <p className="text-[9px] md:text-[10px] mt-2 md:mt-3 font-bold uppercase tracking-[2px] opacity-60">View all your upcoming confirmed trips (Hotel & Flight)</p>
               </div>
             </div>
          </div>
          <div className="flex items-center gap-3 w-full md:w-auto">
             <button onClick={() => navigate("/travel", { state: { activeTab } })} className="w-full md:w-auto px-6 py-3.5 rounded-xl font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-2.5 transition-all shadow-xl bg-[#C9A84C] text-[#000D26] hover:brightness-110 active:scale-95">
