...
function FlightSection() {
  ...
  const formattedBookings = useMemo(() => (flightBookings || []).map(b => {
    ...
    const buildLeg = (segs) => {
      ...
      const resolveCity = (val) => {
        ...
        return val.city || "Unknown";
      };

      return {
        fromCode: resolveCode(first?.origin),
        toCode: resolveCode(last?.destination),
        fromCity: resolveCity(first?.origin),
        toCity: resolveCity(last?.destination)
      };
    };
  ...
  }, [search, startDate, endDate, statusFilter, formattedBookings]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label="My Flights" value={filtered.length} Icon={FaPlane} borderCls="border-[#000D26]" iconBgCls="bg-slate-100" iconColorCls="text-[#000D26]" />
        <StatCard label="Confirmed" value={filtered.filter(b => b.status === "Confirmed").length} Icon={FiCheckCircle} borderCls="border-emerald-500" iconBgCls="bg-emerald-50" iconColorCls="text-emerald-600" />
        ...
        <StatCard label="Total Spent" value={`₹${totalSpend.toLocaleString()}`} Icon={FaRupeeSign} borderCls="border-violet-500" iconBgCls="bg-violet-50" iconColorCls="text-violet-600" />
      </div>

      <div className="bg-white rounded-2xl p-6 border shadow-sm" style={{ borderColor: C.border }}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
          <LabeledField label={<><FiSearch size={10} /> Booking Search</>} className="lg:col-span-4">
              ...
              <tr>
                <td colSpan={7} className="px-6! py-20! text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm font-bold text-slate-400">No bookings found for the selected criteria.</p>
                  </div>
                </td>
              </tr>
            )}
...
function HotelSection() {
  ...
  const filtered = useMemo(() => {
    ...
    return formattedBookings.filter(b => {
      // Basic visibility logic from original MyBookings
      const amendStatus = b?.amendment?.status;
      if (amendStatus && amendStatus !== "not_requested") return false;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label="My Hotels" value={filtered.length} Icon={FaHotel} borderCls="border-[#000D26]" iconBgCls="bg-slate-100" iconColorCls="text-[#000D26]" />
        <StatCard label="Confirmed" value={filtered.filter(b => b.status === "Confirmed").length} Icon={FiCheckCircle} borderCls="border-emerald-500" iconBgCls="bg-emerald-50" iconColorCls="text-emerald-600" />
        <StatCard label="Unique Cities" value={new Set(filtered.map(b => b.city)).size} Icon={FiMapPin} borderCls="border-amber-500" iconBgCls="bg-amber-50" iconColorCls="text-amber-600" />
        <StatCard label="Total Spent" value={`₹${totalSpend.toLocaleString()}`} Icon={FaRupeeSign} borderCls="border-violet-500" iconBgCls="bg-violet-50" iconColorCls="text-violet-600" />
      </div>
                    ...
                    <button 
                      onClick={() => {
                        ...
                        if (elapsed < 120) {
                          ...
                          navigate(`/my-hotel-booking/${b._id}`);
                        }
                      }} 
                      className="p-3 rounded-xl transition-all shadow-sm hover:shadow-md bg-linear-to-br from-[#003399] to-[#000d26] hover:bg-white hover:from-white hover:to-white group"
                    >
...
export default function MyBookings() {
  ...
  useEffect(() => {
    if (typeQuery) setActiveTab(typeQuery === "hotel" ? "hotel" : "flight");
  }, [typeQuery]);

  const handleRefresh = async () => {
    setIsSyncing(true);
    try {
      await dispatch(activeTab === "flight" ? fetchMyBookings() : fetchMyHotelBookings());
    } finally {
      setIsSyncing(false);
    }
  };
    ...
    <div className="min-h-screen font-sans pb-20 -mt-6 -mx-4 md:-mx-6" style={{ background: C.offWhite }}>
        ...
        <div className="w-full flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="flex flex-col md:flex-row md:items-center gap-6">
             ...
             <div className="flex items-center md:items-center gap-4 md:gap-5">
               ...
               <div>
                 <h1 className="text-3xl md:text-4xl font-black tracking-tight leading-none">My Bookings</h1>
                 <p className="text-[9px] md:text-[10px] mt-2 md:mt-3 font-bold uppercase tracking-[2px] opacity-60">View all your bookings (Hotel & Flight)</p>
               </div>
             </div>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
             <button
                onClick={() => navigate("/travel", { state: { activeTab } })}
                className="w-full md:w-auto px-6 py-3.5 rounded-xl font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-2.5 transition-all shadow-xl bg-[#C9A84C] text-[#000D26] hover:brightness-110 active:scale-95"
             >
                {activeTab === "flight" ? <FaPlane size={14} /> : <FaHotel size={14} />}
                New {activeTab === "flight" ? "Flight" : "Hotel"} Booking
             </button>
          </div>
        </div>
      </div>
      ...
      <div className="w-full px-4 md:px-10 -mt-10 space-y-10">
        ...
        <div className="flex gap-2 p-1.5 bg-white border border-slate-200/60 shadow-xl rounded-2xl w-fit max-w-full overflow-x-auto">
           {[["flight", "My Flights", FaPlane], ["hotel", "My Hotels", FaHotel]].map(([k, lbl, Icon]) => (
             <button 
                key={k} 
