...
import { myRejectedRequests } from "../../data/dummyData";
import { FiFilter, FiXCircle, FiMapPin, FiCalendar } from "react-icons/fi";
...
export default function MyRejectedRequests() {
  ...
  const filtered = myRejectedRequests.filter((req) => {
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <table className="w-full text-left">
          <thead style={{ backgroundColor: colors.primary }}>
            <tr>
              {["Type", "Destination", "Dates", "Reason", "Rejected By", "Remarks"].map(
                (h) => (
                  <th
                    key={h}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((req) => (
                <tr key={req.id} className="hover:bg-gray-50 transition">
                  ...
                  {/* Rejected By */}
                  <td className="px-6 py-4 text-sm">{req.rejectedBy}</td>

                  {/* Remarks */}
                  <td className="px-6 py-4 text-sm">
                    <span className="text-red-600 flex items-center gap-1">
                      <FiXCircle /> {req.remarks}
                    </span>
                  </td>
