...
import {
  approvedFlightsExportTemplate,
  approvedHotelsExportTemplate,
  ...
  const formatted = useMemo(() => {
    return flightRaw
      .map((a) => {
        ...
        const travellerName = lead
          ? `${lead.title || ""} ${lead.firstName || ""} ${lead.lastName || ""}`.trim()
          : "Staff Member";
        ...
        const travelDate =
          routes[0]?.departureDate || a.bookingSnapshot?.travelDate;
    ...
    return formatted.filter((a) => {
      ...
      return (
        ...
        (!q ||
          ...
          a.travellerName.toLowerCase().includes(q) ||
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          value={
            filtered.filter((a) => a.executionStatus === "not_started").length
          }
          Icon={FiClock}
        ...
        <StatCard
          label="Failed Exec"
          value={filtered.filter((a) => a.executionStatus === "failed").length}
          Icon={FiX}
        ...
        <StatCard
          label="Total Value"
          value={`₹${filtered.reduce((s, a) => s + a.amount, 0).toLocaleString()}`}
          Icon={FaRupeeSign}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
          <LabeledField
            label={
              <>
                <FiSearch size={10} /> Approval Search
              </>
            }
          ...
          >
            <SearchBar
              value={search}
              onChange={setSearch}
              placeholder="Search by Ref, Employee name..."
            />
          </LabeledField>
      ...
      <ResponsiveDataTable
        ...
        onExport={() =>
          exportExcel({
            pageHeader: "Approved Flight Data",
            statCards: [
              ...
              {
                label: "Failed Exec",
                value: filtered.filter((a) => a.executionStatus === "failed")
                  .length,
              },
              {
                label: "Total Value",
            ...
            {paginated.length > 0 ? (
              paginated.map((a, i) => (
                <tr
                  ...
                  style={{ background: i % 2 === 0 ? C.white : C.gold + "08" }}
                >
                  ...
                  <td className="px-6! py-5!">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase truncate max-w-[120px]">
                      {a.flightRequest?.purposeOfTravel}
                    </p>
                  </td>
                  <td className="px-6! py-5!">
                  ...
                  <td className="px-6! py-5!">
                    <p className="text-xs font-black" style={{ color: C.navy }}>
                      {a.approvedAt
                        ? new Date(a.approvedAt).toLocaleDateString("en-IN", {
                            day: "2-digit",
                            month: "short",
                            year: "numeric",
                          })
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase">
                      {a.approvedAt
                        ? new Date(a.approvedAt).toLocaleTimeString("en-IN", {
                            hour: "2-digit",
                            minute: "2-digit",
                          })
                        : ""}
                  ...
                  <td className="px-6! py-5!">
                    <p className="text-xs font-black" style={{ color: C.navy }}>
                      {a.travelDate
                        ? new Date(a.travelDate).toLocaleDateString("en-IN", {
                            day: "2-digit",
                            month: "short",
                            year: "numeric",
                          })
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase">
                      {a.travelDate
                        ? new Date(a.travelDate).toLocaleTimeString("en-IN", {
                            hour: "2-digit",
                            minute: "2-digit",
                          })
                        : ""}
                  ...
                  <td className="px-6! py-5!">
                    {a.requestStatus === "manager_approved" ? (
                      <span className="bg-amber-100 text-amber-600 text-[9px] font-black px-2 py-1 rounded border border-amber-200 uppercase tracking-tight">
                        Awaiting Travel Admin Approval
                      </span>
                    ) : (
                      <ExecStatusBadge
                        status={
                          isDiscarded(a.travelDate)
                            ? "discarded"
                            : a.executionStatus
                        }
                      />
                  ...
                  <td className="px-6! py-5! text-center!">
                    <button
                      ...
                      className="p-3 rounded-xl transition-all shadow-sm hover:shadow-md bg-linear-to-br from-[#003399] to-[#000d26] hover:from-slate-800 group"
                    >
                      <FiEye
                        size={16}
                        className="text-[#E7C695] group-hover:scale-110 transition-transform"
                      />
                    </button>
                  </td>
                </tr>
      ...
      {selected && (
        <PendingFlightDetailsModal
          ...
          isVerified={true}
          isDiscarded={isDiscarded(selected.travelDate)}
        />
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
          ...
          >
            <SearchBar
              value={search}
              onChange={setSearch}
              placeholder="Search by Hotel, Employee name..."
            />
          </LabeledField>
          <LabeledField label="Execution" className="lg:col-span-2">
            <CustomDropdown
      ...
      <ResponsiveDataTable
        ...
        onExport={() =>
          exportExcel({
            pageHeader: "Approved Hotel Data",
            statCards: [
              ...
              paginated.map((a, i) => (
                ...
                >
                  ...
                  <td className="px-6! py-5!">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase truncate max-w-[120px]">
                      {a.hotelRequest?.purposeOfTravel}
                    </p>
                  </td>
                  <td className="px-6! py-5!">
                    <span
                  ...
                  <td className="px-6! py-5!">
                    <p className="text-xs font-black" style={{ color: C.navy }}>
                      {a.checkIn
                        ? new Date(a.checkIn).toLocaleDateString("en-IN", {
                            day: "2-digit",
                            month: "short",
                            year: "numeric",
                          })
      ...
      {selected && (
        <PendingHotelDetailsModal
          ...
          onClose={() => setSelected(null)}
          isVerified={true}
          isDiscarded={isDiscarded(selected.checkIn)}
        />
      )}
    </div>
  );
    ...
    >
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`p-3.5 rounded-2xl bg-white/10 transition-all border border-white/10 backdrop-blur-md shadow-xl ${isSyncing || loadingActive ? "opacity-50 cursor-not-allowed" : "hover:bg-white/20"}`}
                disabled={isSyncing || loadingActive}
              >
                <div
                  className={isSyncing || loadingActive ? "animate-spin" : ""}
                >
                  <FiRefreshCw size={22} />
                </div>
              </button>
            ...
            <div className="flex items-center md:items-center gap-4 md:gap-5">
              <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl flex shrink-0 items-center justify-center shadow-2xl text-white border border-white/20 bg-white/10 backdrop-blur-md">
                <FiCheckCircle size={28} className="md:w-8 md:h-8" />
              </div>
              <div>
                ...
                <p className="text-[10px] md:text-[11px] mt-2 md:mt-3 font-bold uppercase tracking-[2px] md:tracking-[3px] opacity-60">
                  Requests approved and waiting for booking
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="w-full px-4 md:px-10 -mt-12 space-y-10">
        <div className="flex gap-2 p-1.5 bg-white border border-slate-200/60 shadow-xl rounded-2xl w-fit max-w-full overflow-x-auto">
          {[
            ["flight", "Approved Flights", FaPlane, flightCount],
            ["hotel", "Approved Hotels", FaHotel, hotelCount],
          ].map(([k, lbl, Icon, count]) => (
            <button
              key={k}
            ...
            >
              ...
              <span
                className={`ml-1 px-2 py-0.5 rounded-full text-[9px] ${activeTab === k ? "bg-white/20 text-white" : "bg-slate-100 text-slate-400"}`}
              >
                {count}
              </span>
            </button>
