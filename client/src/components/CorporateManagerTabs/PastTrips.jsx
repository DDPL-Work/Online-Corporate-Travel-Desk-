...
import {
  pastFlightsExportTemplate,
  pastHotelsExportTemplate,
  ...
  const formatted = useMemo(() => {
    return (flightBookings || [])
      .map((b) => {
        ...
        const travelDate =
          routes[0]?.departureDate || b.bookingSnapshot?.travelDate;
        const travelKey = travelDate
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
          <LabeledField
            label={
              <>
                <FiSearch size={10} /> History Search
              </>
            }
                ...
                >
                  ...
                  <td className="!px-6 !py-5 !text-center">
                    <button
                      ...
                      className="p-3 rounded-xl transition-all shadow-sm hover:shadow-md bg-gradient-to-br from-[#003399] to-[#000d26] hover:from-slate-800 group"
                    >
                      <FiEye
                        size={16}
  ...
  const formatted = useMemo(() => {
    return (hotelBookings || [])
      .filter((b) => {
        ...
        return (
          validStatuses.includes(executionStatus) &&
          ci < today &&
          !isCancelled &&
          !isFailed &&
          !isAmended
              ...
              <tr>
                <td colSpan={7} className="!px-6 !py-20 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm font-bold text-slate-400">
                      No past hotel stay records found.
                    </p>
                  </div>
                </td>
              </tr>
...
export default function PastTripsForManager() {
  ...
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { loadingTeamExecutedFlightRequests: loadingFlights } = useSelector(
    (state) => state.manager,
  );
    ...
    >
            ...
            <div className="flex items-center md:items-center gap-4 md:gap-5">
              ...
              <div>
                ...
                <p className="text-[10px] md:text-[11px] mt-2 md:mt-3 font-bold uppercase tracking-[2px] md:tracking-[3px] opacity-60">
                  View all the past trips (Hotel & Flight) which were done by your team
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="w-full px-4 md:px-10 -mt-12 space-y-10">
        <div className="flex gap-2 p-1.5 bg-white border border-slate-200/60 shadow-xl rounded-2xl w-fit max-w-full overflow-x-auto">
          {[
            ["flight", "Flight Trips", FaPlane],
            ["hotel", "Hotel Trips", FaHotel],
          ].map(([k, lbl, Icon]) => (
            <button
              key={k}
