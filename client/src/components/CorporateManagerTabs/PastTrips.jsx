...
function FlightSection() {
  ...
  const formatted = useMemo(() => {
    return (flightBookings || []).map(b => {
      ...
      const requestStatus = (b.requestStatus || "").toLowerCase();
      const isAmended = b.amendment && (b.amendment.status === "requested" || b.amendment.status === "in_progress");

      const status = (executionStatus === "cancelled" || executionStatus === "cancel_requested" || requestStatus === "cancelled") ? "Cancelled" : ((executionStatus === "ticketed" || executionStatus === "confirmed") && executionStatus !== "failed" && !isAmended) ? "Completed" : "Pending";
    ...
    }).filter(t => t.travelKey && t.travelKey < today && t.status === "Completed");
  }, [flightBookings, today]);

  const filtered = useMemo(() => {
    const q = search.toLowerCase();
    return formatted.filter(b => {
      const bDate = b.travelDate ? new Date(b.travelDate).toISOString().slice(0, 10) : "";
      return (!q || b.travellerName.toLowerCase().includes(q) || (b.orderId || "").toLowerCase().includes(q) || b.pnr.toLowerCase().includes(q) || b.employeeId.toLowerCase().includes(q)) &&
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard label="Last 30 Days" value={filtered.filter(t => t.travelKey >= new Date(Date.now() - 30*86400000).toISOString().slice(0,10)).length} Icon={FiCalendar} borderCls="border-amber-500" iconBgCls="bg-amber-50" iconColorCls="text-amber-600" />
        <StatCard label="Total Records" value={filtered.length} Icon={FiList} borderCls="border-violet-500" iconBgCls="bg-violet-50" iconColorCls="text-violet-600" />
      </div>

      <div className="bg-white rounded-2xl p-6 border shadow-sm" style={{ borderColor: C.border }}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
          <LabeledField label={<><FiSearch size={10} /> History Search</>} className="lg:col-span-4">
            ...
            {paginated.length > 0 ? paginated.map((b, i) => (
              <tr key={b._id} className="hover:bg-slate-100 transition-colors" style={{ background: i % 2 === 0 ? C.white : C.gold + "08" }}>
                ...
                <td className="!px-6 !py-5 !text-center">
                    <button onClick={() => navigate(`/manager/team-booking/${b._id}`, { state: { isPastTrip: true } })} className="p-3 rounded-xl transition-all shadow-sm hover:shadow-md bg-gradient-to-br from-[#003399] to-[#000d26] hover:from-slate-800 group">
              ...
              <tr>
                <td colSpan={8} className="!px-6 !py-20 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm font-bold text-slate-400">No past flight records found.</p>
                  </div>
                </td>
              </tr>
            )}
...
function HotelSection() {
  ...
  const formatted = useMemo(() => {
    return (hotelBookings || [])
      .filter((b) => {
        ...
        const executionStatus = (b.executionStatus || "").toLowerCase();
        const requestStatus = (b.requestStatus || "").toLowerCase();
        const isCancelled = executionStatus === "cancelled" || executionStatus === "cancel_requested" || requestStatus === "cancelled" || (b.amendment && ["requested", "success"].includes(b.amendment?.status)) || b.isCancelled === true;
        const isFailed = executionStatus === "failed";
        const isAmended = b.amendment && (b.amendment.status === "requested" || b.amendment.status === "in_progress");

        return validStatuses.includes(executionStatus) && ci < today && !isCancelled && !isFailed && !isAmended;
      })
      .map((b) => {
        ...
        return {
          ...
          checkIn,
          checkOut,
          checkInISO: new Date(checkIn).toISOString().slice(0, 10)
        };
      });
  }, [hotelBookings, today]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label="Archived Stays" value={filtered.length} Icon={FaHotel} borderCls="border-[#000D26]" iconBgCls="bg-slate-100" iconColorCls="text-[#000D26]" />
        <StatCard label="Completed" value={filtered.length} Icon={FiCheckCircle} borderCls="border-emerald-500" iconBgCls="bg-emerald-50" iconColorCls="text-emerald-600" />
        <StatCard label="Total Records" value={filtered.length} Icon={FiList} borderCls="border-violet-500" iconBgCls="bg-violet-50" iconColorCls="text-violet-600" />
        <StatCard label="History Count" value={filtered.length} Icon={FiRefreshCw} borderCls="border-amber-500" iconBgCls="bg-amber-50" iconColorCls="text-amber-600" />
      </div>
              ...
              <tr>
                <td colSpan={7} className="!px-6 !py-20 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm font-bold text-slate-400">No past hotel stay records found.</p>
                  </div>
                </td>
              </tr>
            )}
...
export default function PastTripsForManager() {
  ...
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { loadingTeamExecutedFlightRequests: loadingFlights } = useSelector((state) => state.manager);
        ...
        <div className="w-full flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="flex items-center gap-6">
             ...
             <div className="flex items-center gap-5">
               ...
               <div>
                 <h1 className="text-4xl font-black tracking-tight leading-none">Past Trips</h1>
                 <p className="text-[11px] mt-3 font-bold uppercase tracking-[3px] opacity-60">Archival View of Completed Travel</p>
               </div>
             </div>
          </div>
        </div>
