...
export default function PendingCancellationRequestsForManager() {
  const dispatch = useDispatch();
  const { managerRequests, isLoading: isSyncing } = useSelector((state) => state.amendmentRequest);
  ...
  const pendingFlightRequests = useMemo(() => {
    return (managerRequests || []).filter(r => r.bookingType === "flight");
  }, [managerRequests]);
  
  const pendingHotelRequests = useMemo(() => {
    return (managerRequests || []).filter(r => r.bookingType === "hotel");
  }, [managerRequests]);
  ...
  const handleRefresh = async () => {
    dispatch(getManagerAmendmentRequests());
  };
        ...
        <div className="w-full flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="flex flex-col md:flex-row md:items-center gap-6">
             ...
             <div className="flex items-center md:items-center gap-4 md:gap-5">
               ...
               <div>
                 <h1 className="text-3xl md:text-4xl font-black tracking-tight leading-none">Team Pending Cancellations</h1>
                 <p className="text-[10px] md:text-[11px] mt-2 md:mt-3 font-bold uppercase tracking-[2px] md:tracking-[3px] opacity-60">View all the pending cancellation requests which were done by your team</p>
               </div>
             </div>
          </div>
        </div>
