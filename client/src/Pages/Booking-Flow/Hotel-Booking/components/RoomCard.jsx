...
const RoomCard = ({ room, onSelect }) => {
  ...
  const perNight = price.RoomPrice
    ? price.RoomPrice / nights
    : (totalFare - tax) / nights;
  ...
  const getInclusionIcon = (text) => {
    ...
    if (
      ...
      return <RiRestaurant2Line className="text-amber-500" />;
    ...
    if (lower.includes("cancel") || lower.includes("refundable"))
      return <FaShieldAlt className="text-emerald-500" />;
      ...
      <div className="flex flex-col lg:flex-row h-full">
        ...
        <div className="w-full lg:w-72 shrink-0 relative bg-slate-100 overflow-hidden">
          ...
          {totalImages > 1 && (
            <div className="absolute top-1/2 -translate-y-1/2 w-full px-2 flex justify-between opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                onClick={prev}
                className="w-8 h-8 rounded-full bg-white/90 backdrop-blur shadow-lg flex items-center justify-center text-slate-900 hover:bg-white transition-colors"
              >
                ‹
              </button>
              <button
          ...
          <div className="absolute top-3 left-3">
            <span
              className={`text-[10px] font-black px-2 py-1 rounded-lg uppercase tracking-widest shadow-lg ${room.IsRefundable ? "bg-emerald-500 text-white" : "bg-slate-700 text-white"}`}
            >
            ...
            {inclusions.length > 4 && (
              ...
              >
                {showDetails ? (
                  ...
                  <>
                    <MdExpandMore className="text-base" /> VIEW{" "}
        ...
        <div className="w-full lg:w-64 p-5 bg-slate-50/50 flex flex-col justify-between md:items-end">
          <div className="md:text-right space-y-1">
            ...
            <div className="flex items-center md:justify-end gap-1.5 text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded-full ">
