...
/* ─── Date Field ─── */
const DateField = ({ label, value, onChange, min, open, setOpen, align = "left" }) => {
  const fmt = (dateStr) => {
    ...
    <div className="relative w-full">
      ...
      >
        ...
        <div className="flex flex-col flex-1 min-w-0">
          <span className="text-[10px] font-bold uppercase tracking-widest leading-none" style={{ color: ORANGE }}>{label}</span>
          {f ? (
      ...
      {open && (
        <div className={`absolute top-[calc(100%+8px)] z-[300] ${align === "right" ? "right-0" : "left-0"}`}>
          <CustomCalendar value={value} minDate={min} onChange={(val) => { onChange(val); setOpen(false); }} onClose={() => setOpen(false)} />
        </div>
      )}
    </div>
  );
...
const Header = () => {
  ...
  useEffect(() => {
    const handler = (e) => { if (guestRef.current && !guestRef.current.contains(e.target)) setShowGuestDropdown(false); };
  ...
  useEffect(() => {
    setRoomConfigs((prev) => {
      const next = [...prev];
      if (form.rooms > next.length) { for (let i = next.length; i < form.rooms; i++) next.push({ adults: 2, children: 0, childrenAges: [] }); }
  ...
  const normalizedCountries = Array.isArray(countryArray) ? countryArray.map((c) => { const code = c.Code || c.code; return { code, name: c.Name || c.name, flag: getFlagEmoji(code) }; }) : [];

  const handleSearch = () => {
    if (!form.cityCode || !form.checkIn || !form.checkOut) return;
    dispatch(searchHotels({ payload: { CheckIn: form.checkIn, CheckOut: form.checkOut, CityCode: form.cityCode, GuestNationality: form.guestNationality, NoOfRooms: form.rooms, PaxRooms: roomConfigs.map((r) => ({ Adults: r.adults, Children: r.children, ChildrenAges: (r.childrenAges || []).slice(0, r.children) })), IsDetailedResponse: true, Filters: { Refundable: true, MealType: "All" }, ResponseTime: 23 }, page: 1, limit: 10 }));
  };
    ...
    <header className="bg-[#000D26] shadow-lg">
      <div className="container mx-auto px-4 pt-3 pb-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <button type="button" onClick={() => { const slug = location.state?.companySlug || publicBranding?.companySlug; if (slug) navigate(`/travel`); else window.history.back(); }} className="flex items-center gap-1.5 text-sm font-semibold text-white/60 hover:text-white transition">
            <MdArrowBack /> Back to Search
          </button>
          <button type="button" onClick={() => { navigate(`/travel`, { state: { activeTab: "flight" } }); }} className="flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg transition shadow-sm" style={{ background: ORANGE, color: DARK }}>
        ...
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          ...
          {form.cityName && (<><span className="text-white/50 text-sm">in</span><span className="text-white font-bold">{form.cityName}</span></>)}
          {nights > 0 && (<span className="text-xs font-black px-2.5 py-0.5 rounded-full" style={{ background: ORANGE, color: DARK }}>{nights} Night{nights !== 1 ? "s" : ""}</span>)}
        </div>
          ...
          <div className="flex-[1.3] min-w-0 relative" ref={guestRef}>
            <button type="button" onClick={() => setShowGuestDropdown((v) => !v)} className="w-full h-full flex items-center gap-2 px-4 py-3 bg-amber-50/40 border border-amber-100/60 text-left transition-colors rounded-xl cursor-pointer hover:bg-amber-50">
              ...
              <span className="flex flex-col flex-1 min-w-0">
                <span className="text-[10px] font-bold uppercase tracking-widest leading-none" style={{ color: ORANGE }}>Guests &amp; Rooms</span>
                <span className="text-sm font-bold text-gray-800 mt-1">{totalGuests} Guest{totalGuests !== 1 ? "s" : ""}, {rooms} Room{rooms !== 1 ? "s" : ""}</span>
              </span>
        ...
        {(form.cityName || nights > 0) && (
          <div className="flex items-center gap-4 mt-2 px-1 flex-wrap">
            {form.cityName && (<span className="text-white/50 text-xs font-medium flex items-center gap-1"><MdLocationOn className="text-xs" /> {form.cityName}</span>)}
            {nights > 0 && (<span className="text-white/50 text-xs font-medium">{nights} Night{nights !== 1 ? "s" : ""} · {form.rooms} Room{form.rooms !== 1 ? "s" : ""} · {totalGuests} Guest{totalGuests !== 1 ? "s" : ""}</span>)}
          </div>
        )}
      </div>
    </header>
