...
const airportDisp = (code) => {
  const a = getAirport(code);
  return a ? `${a.city}, ${a.country}` : code || "—";
};
const airportSub = (code) => {
  const a = getAirport(code);
  return a ? `${code} · ${a.name?.slice(0, 28)}` : "";
};
...
const derive = (payload, journeyType) => {
  ...
  const adults =
    payload?.AdultCount ?? payload?.adults ?? payload?.passengers?.adults ?? 1;
  const children =
    payload?.ChildCount ??
    payload?.children ??
  ...
  const infants =
    ...
    0;
  const cabRaw =
    payload?.Segments?.[0]?.FlightCabinClass ??
    payload?.cabinClass ??
    "economy";
  const cabin =
    typeof cabRaw === "number" ? CABIN_TBO[cabRaw] || "economy" : cabRaw;
  const origin = payload?.Segments?.[0]?.Origin || payload?.origin || "";
  const destination =
    payload?.Segments?.[0]?.Destination || payload?.destination || "";
  const dep = toISO(
    payload?.Segments?.[0]?.PreferredDepartureTime || payload?.departureDate,
  );
  const ret = toISO(
    payload?.Segments?.[1]?.PreferredDepartureTime ||
      payload?.returnDate ||
      payload?.ReturnDate,
  );
...
const buildPayload = (draft, original) => {
  ...
  const counts = {
    AdultCount: draft.passengers.adults,
    ChildCount: draft.passengers.children,
    InfantCount: draft.passengers.infants,
  };
      ...
      Segments: [
        {
          ...
          FlightCabinClass: cc,
          PreferredDepartureTime: draft.departureDate
            ? `${draft.departureDate}T00:00:00`
            : "",
        },
      ],
    };
    ...
    return {
      ...
      Segments: [
        ...
        },
        {
          ...
          FlightCabinClass: cc,
          PreferredDepartureTime: draft.returnDate
            ? `${draft.returnDate}T00:00:00`
            : "",
        },
      ],
    };
  if (jt === 3)
    return {
      ...
      Segments: (draft.segments || []).map((s, i) => ({
        ...
        FlightCabinClass: cc,
        PreferredDepartureTime: s.departureDate
          ? `${s.departureDate}T00:00:00`
          : "",
      })),
    };
  return original;
        ...
        >
          ...
          {hits.map((a, i) => (
            ...
            >
              ...
              <span className="text-[12px] font-bold text-[#C9A84C] bg-slate-100 px-2 py-0.5 rounded-md">
                {a.iata_code}
              </span>
            </div>
          ))}
    ...
    <div ref={ref} className="relative cursor-pointer">
      ...
      {isOpen && (
        <TravelersClassModal
          onClose={onToggle}
          onApply={(data) => {
            const selectedOpt = CABIN_OPTIONS.find(
              (o) => o.label === data.travelClass,
            );
            onChange({
              passengers: {
                adults: data.adults,
...
function TripTypeDropdown({ value, onChange, journeyLabel, isOpen, onToggle }) {
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div
          className="absolute top-[calc(100%+8px)] left-0 w-full min-w-[160px] bg-white rounded-xl shadow-2xl border border-gray-100 z-[1001] overflow-hidden"
...
export default function ResearchableFlightHeader({
  ...
  const journeyLabel =
    jt === 2 ? "Round Trip" : jt === 3 ? "Multi City" : "One Way";
      ...
      setDraft((d) => ({
        ...
        segments: [
          ...
          },
          {
            ...
            destination: "",
            departureDate: d.returnDate || d.departureDate,
          },
        ],
      }));
    }
    ...
    <div className="flex items-stretch gap-2 py-2.5 sm:py-3 w-full animate-fadeIn transition-colors duration-200 flex-wrap">
        ...
        {openDropdown === "dep" && (
          ...
          >
            <CustomCalendar
              ...
              onChange={(val) => {
                setDraft({ ...draft, departureDate: val });
                setOpenDropdown(null);
              }}
      ...
      <div className="w-full sm:w-auto flex flex-col shrink-0 min-w-[140px] sm:min-w-[160px] px-2.5 sm:px-3.5 py-1.5 sm:py-2 border border-slate-200 bg-slate-50 rounded-[8px] shadow-sm">
        ...
        <PaxCabin
          ...
          onToggle={() =>
            setOpenDropdown(openDropdown === "pax" ? null : "pax")
          }
          onChange={({ passengers, cabin }) => {
            setDraft((d) => ({ ...d, passengers, cabin }));
            setOpenDropdown(null);
          }}
        />
      </div>
  ...
  const renderMultiCity = () => (
    <div className="p-3 sm:p-4 px-3 sm:px-5 transition-colors duration-200 flex flex-col gap-3 bg-slate-50">
      <div className="flex w-full items-center">
        <TripTypeDropdown
          value={jt}
          onChange={(v) => setDraft({ ...draft, journeyType: v })}
          journeyLabel={journeyLabel}
        />
      </div>
                ...
                {openDropdown === `mc-${idx}` && (
                  ...
                  >
                    <CustomCalendar
                      ...
                      minDate={
                        idx > 0
                          ? draft.segments[idx - 1]?.departureDate
                          : new Date().toISOString().split("T")[0]
                      }
                      onChange={(val) => {
                        const s = [...draft.segments];
                        s[idx] = { ...s[idx], departureDate: val };
                        setDraft((d) => ({ ...d, segments: s }));
                        setOpenDropdown(null);
                      }}
                      onClose={() => setOpenDropdown(null)}
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 sm:gap-0 pt-2">
        <div className="flex flex-col px-3.5 py-1.5 border border-slate-200 bg-slate-50 rounded-[8px] sm:min-w-[180px]">
          ...
          <PaxCabin
            ...
            onToggle={() =>
              setOpenDropdown(openDropdown === "pax-mc" ? null : "pax-mc")
            }
            onChange={({ passengers, cabin }) => {
              setDraft((d) => ({ ...d, passengers, cabin }));
              setOpenDropdown(null);
