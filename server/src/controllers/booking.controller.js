...
exports.downloadTicketPdf = asyncHandler(async (req, res) => {
  ...
  const pdfPath = await pdfService.generateFlightTicketPdf({
    booking: {
      ...booking.toObject(),
      reissueMeta: booking.isReissued
        ? {
            isReissued: true,
            headerTitle: "REISSUED E-TICKET",
            originalPnr: booking?.servicing?.reissue?.originalPnr || pnr,
            generatedAt: booking.updatedAt,
            reissueReferenceId:
              booking?.servicing?.reissue?.currentRequestId?.toString?.() ||
              booking?.servicing?.reissue?.offlineRequestId?.toString?.() ||
              null,
          }
        : null,
    },
