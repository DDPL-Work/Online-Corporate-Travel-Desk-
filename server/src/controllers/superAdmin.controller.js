...
const { getAgencyBalance } = require("../services/tboBalance.service.js");

// ======================================================
// GET TBO AGENCY BALANCE
// ======================================================
exports.getTboAgencyBalance = async (req, res) => {
  try {
    const { env = "dummy" } = req.body;
    ...
    console.error("TBO Balance Error:", err.message);
    res.status(500).json({
      success: false,
      message: err.message || "Failed to fetch agency balance"
    });
  }
};
...
exports.deactivateCorporateAdmin = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ success: true, message: "Corporate Admin deactivated" });
  } catch (err) {
    next(err);
  }
};
...
exports.activateCorporateAdmin = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ success: true, message: "Corporate Admin activated" });
  } catch (err) {
    next(err);
  }
};
...
exports.removeCorporateAdmin = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ success: true, message: "Corporate Admin removed" });
  } catch (err) {
    next(err);
  }
};